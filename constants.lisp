...
(defmacro define-color (name hex &optional package)
  (let* ((package (or (etypecase package
                        (package package)
                        (null (find-package '#:org.shirakumo.alloy.colored.colors))
                        ((eql T) *package*)
                        (string (find-package package))
                        (symbol (find-package package)))
                      (error "No such package ~s" package)))
         (name (intern (string name) package)))
    `(progn
       (export ',name ,package)
       (unless (boundp ',name)
         (defconstant ,name (rgb ,hex))))))
